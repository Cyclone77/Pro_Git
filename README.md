...
``` git 
//输出详细的提交历史,显示：提交的 SHA-1 校验、作者，日期，邮箱以及提交说明
$ git log

//可以用带参数的`git log`， 一个常用的选项是 `-p`，用来显示每次提交的内容差异。 你也可以加上 `-2` 来仅显示最近两次提交：
$ git log -p -2

//可以查看简略的内容差异
$ git log --stat

//另外一个常用的选项是 --pretty。 这个选项可以指定使用不同于默认格式的方式展示提交历史。oneline 将每个提交放在一行显示。
